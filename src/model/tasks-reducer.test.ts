...
import {changeTaskAC, createTaskAC, deleteTaskAC, tasksReducer} from "./tasks-reducer.ts";
...
test('correct task should change its status', () => {
    const endState = tasksReducer(
        startState,
        changeTaskAC({todolistId: 'todolistId2', taskId: '2', updateFields: {isDone: false}})
    )
...
test('correct task should change its title', () => {
    const endState = tasksReducer(
        startState,
        changeTaskAC({todolistId: 'todolistId2', taskId: '2', updateFields: {title: 'juice'}})
    )
